...
// Parse Authorization Code from URL and exchange it for a token
...
// Redirect user to AniList OAuth Page
...
async function renderModalLibrarySection(anime) {
  ...
  if (!anime || !state.accessToken) {
    // If logged out, clicking the heart alerts the user to log in
  ...
  // Query AniList to check if this anime is in the user's watchlist
  const query = `
    query ($mediaId: Int) {
      Media(id: $mediaId) {
        mediaListEntry {
  ...
  // Bind click event to toggle watchlist
  const cleanFavBtn = favBtn.cloneNode(true);
  favBtn.replaceWith(cleanFavBtn);
...
// Toggle Watchlist list entry (Add current or delete)
async function toggleWatchlist(mediaId) {
  ...
  try {
    if (inList) {
      ...
      // Add as current Watchlist entry (status: CURRENT)
      const query = `
