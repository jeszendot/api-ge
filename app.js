if (typeof window === 'undefined') {
  // ==========================================================================
  // NODE.JS UNIFIED BACKEND SERVER (Runs in terminal via "node app.js")
  // ==========================================================================
  ...
  http.createServer((req, res) => {
    ...
    // Proxy Token Exchange POST requests
...
/* ==========================================================================
   ANIVIBE - JAVASCRIPT CONTROLLER (Browser client)
   ========================================================================== */
...
const state = {
  ...
  user: null,
  activeAnimeLibraryEntry: null // { id, status, progress, score, isFavourite }
};
...
// AniList GraphQL Query: Discover Media (Provided by User)
const GRAPHQL_QUERY = `
...
// Helper: Convert hex color to RGB values for transparent overlays
function hexToRgb(hex) {
  if (!hex) return '59, 130, 246'; // Fallback to theme blue
...
// ==========================================================================
// OAUTH2 AUTHENTICATION PIPELINE
// ==========================================================================

// Parse Authorization Code from URL and exchange it for a token (Authorization Code Flow)
async function parseAuthCode() {
  const urlParams = new URLSearchParams(window.location.search);
  const code = urlParams.get('code');
  if (code) {
    // Clear code from URL search parameter immediately
    window.history.replaceState({}, document.title, window.location.pathname);
    await exchangeCodeForToken(code);
  }
}

// Post-exchange code helper
...
// Redirect user to AniList OAuth Page using Authorization Code Grant
...
// Fetch authenticated viewer profile details
...
// Render Top Header Authentication panel
function renderAuthPanel() {
  ...
  if (state.user) {
    ...
    // Attach listener to dynamically created logout btn
...
// ==========================================================================
// CORE API FETCH & RENDERING DIRECTORY
// ==========================================================================
...
async function fetchAnime() {
  ...
  try {
    ...
    state.error = err.message || 'Something went wrong while connecting to AniList.';
  } finally {
    state.loading = false;
    renderState();
  }
...
function renderState() {
  ...
  // Click bindings for modal opening
...
// ==========================================================================
// DETAILS MODAL & ANILIST MUTATIONS CONTROLLERS
// ==========================================================================
...
async function openModal(anime) {
  ...
  // Format / Studio / Season Info list
  ...
  // Load Library status if authenticated
...
// Render details modal library / watchlist heart button status and rating stars
async function renderModalLibrarySection(anime) {
  ...
  // Always initialize empty entry state
...
// Update Watchlist Heart UI icon
...
// Toggle Watchlist entry (supports both guest and AniList-authenticated users)
async function toggleWatchlist(mediaId) {
  ...
  if (!state.accessToken) {
    ...
    if (inList) {
      const updated = list.filter(e => e.mediaId !== mediaId);
  ...
  try {
    if (inList) {
      ...
      const response = await fetch(ANILIST_API_URL, {
        ...
        body: JSON.stringify({ query, variables: { id: state.activeAnimeLibraryEntry.id } })
      });
      const result = await response.json();
      if (response.ok) {
        state.activeAnimeLibraryEntry = { id: null, status: 'REMOVE', score: 0 };
      ...
      const query = `
        mutation ($mediaId: Int, $status: MediaListStatus) {
          SaveMediaListEntry(mediaId: $mediaId, status: $status) { id status score(format: POINT_100) }
        }
      `;
      const response = await fetch(ANILIST_API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': `Bearer ${state.accessToken}` },
        body: JSON.stringify({ query, variables: { mediaId, status: 'CURRENT' } })
      });
      const result = await response.json();
...
// Save rating (supports both guest and AniList-authenticated users)
async function saveRating(mediaId, scoreValue) {
  ...
  try {
    const query = `
      mutation ($mediaId: Int, $status: MediaListStatus, $scoreRaw: Int) {
        SaveMediaListEntry(mediaId: $mediaId, status: $status, scoreRaw: $scoreRaw) {
          id status score(format: POINT_100)
        }
      }
    `;
    ...
    const response = await fetch(ANILIST_API_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': `Bearer ${state.accessToken}` },
      body: JSON.stringify({ query, variables: { mediaId, status: currentStatus, scoreRaw: scoreValue } })
    });
    const result = await response.json();
    if (response.ok && result.data && result.data.SaveMediaListEntry) {
      const entry = result.data.SaveMediaListEntry;
